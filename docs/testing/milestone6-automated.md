...
- OpenAPI: regenerated `docs/openapi.json` must match `apps/api/src/openapi.ts`; run `git diff --exit-code -- docs/openapi.json` after generation.
...
- CI workflow changes when Docker build or generated-artifact validation changes.
