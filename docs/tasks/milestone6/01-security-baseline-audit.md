...
- [x] Web inventory covers NextAuth session settings, API proxy routes, cron proxy auth, cookie handling, and any dev-only bypass behavior.
- [x] Environment inventory confirms all required production values are documented in `infra/env/*.example`, README, and production runbooks.
...
- Web API routes are limited to NextAuth, logout, `/api/auth/me`, and the digest cron proxy. There are no general task/tag API proxy routes; app data clients call the API base URL directly.
...
- The web session bridge sets the API session cookie as `httpOnly`, `sameSite: 'lax'`, `secure` in production, path `/`, seven-day max age, and optional `COOKIE_DOMAIN`; the API auth routes use matching cookie semantics.
...
- `docs/tasks/milestone6/02-cors-cookie-proxy-hardening.md`: added follow-ups for CORS fallback decisions, explicit cookie topology, `TRUST_PROXY` tests, README `TRUST_PROXY` docs, and API error handling.
- `docs/tasks/milestone6/03-rate-limit-and-abuse-controls.md`: added follow-ups for explicit body-size limits, mutation/query abuse checks, endpoint-specific rate-limit coverage, job-secret negative tests, and trusted-proxy/IP spoofing tests.
- `docs/tasks/milestone6/05-docs-adrs-readme-refresh.md`: added follow-ups to document `TRUST_PROXY`, production placeholders, no general web task/tag proxy, and email scheduled-send gates.
- `docs/tasks/milestone6/07-security-http-pack-and-smoke.md`: added follow-ups for CORS/cookie manual checks, cron/job auth negative checks, and safe bounded rate-limit smokes.
- `docs/tasks/milestone6/08-milestone6-verification.md`: added release-gate checks that no audit follow-up remains only in this document and production email gates remain explicit.
