...
## Acceptance Criteria
- [x] CI builds the API Docker image from the repo root or documented build context.
- [x] CI builds the web Docker image from the repo root or documented build context.
- [x] Docker build steps use safe placeholder environment values and never require Resend, OAuth, or production secrets.
- [x] `docker compose -f infra/docker-compose.yml config --quiet` runs in CI or an equivalent validation step exists.
