...
describe('email digest routes', () => {
  ...
  it('enforces a stricter rate limit for manual sends and echoes idempotency key header', async () => {
    const prisma = {
      task: { findMany: jest.fn().mockResolvedValue([]) },
      emailPreference: { findUnique: jest.fn().mockResolvedValue({ dailyDigestEnabled: true, dailyDigestTimezone: 'UTC' }) },
    } as unknown as PrismaClient;
    const { app } = appWithUser(prisma, jest.fn().mockResolvedValue({ digestDate: '2026-05-21', attempted: 1 }));

    const first = await request(app)
      .post('/email/digest/send')
      .send({ digestDate: '2026-05-21', dryRun: true, idempotencyKey: 'manual-send-2026-05-21' })
      .expect(200);
    expect(first.headers['x-taskforge-idempotency-key']).toBe('manual-send-2026-05-21');
