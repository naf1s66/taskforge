...
The API session cookie is `httpOnly`, `SameSite=Lax`, seven days long, and `Secure` when `NODE_ENV=production`. Leave `COOKIE_DOMAIN` unset for same-host or host-only deployments. For deliberate multi-subdomain deployments (for example `api.example.com` and `app.example.com`), set the same parent `COOKIE_DOMAIN` such as `.example.com` in both `.env` files so session cookies are shared correctly. Preview deployments are not trusted automatically; add their exact origins to `CORS_ALLOWED_ORIGINS` or keep them isolated from the production API. See `docs/prod/browser-auth-deployment.md` for the full deployed browser checklist.
